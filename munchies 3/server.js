...
app.get('/api/me', requireAuth(), (req, res) => {
  const u = db.prepare('SELECT id,email,name,role,loyalty_points,loyalty_tier,phone,age_verified,created_at FROM users WHERE id=?').get(req.user.id);
  res.json({ user: u });
});
...
function cartView(userId) {
  const rows = db.prepare(`
    SELECT ci.id as item_id, ci.qty, v.id as variant_id, v.size, v.price_cents,
           p.id as product_id, p.name, p.emoji, p.sub, p.type
    FROM cart_items ci
...
app.post('/api/admin/products', requireAuth(['admin']), (req, res) => {
  const { name, sub, emoji, category_id, type, thc, cbd, description, tag, variants } = req.body || {};
  if (!name || !category_id) return res.status(400).json({ error: 'name & category required' });
  const r = db.prepare(`INSERT INTO products (name,sub,emoji,category_id,type,thc,cbd,description,tag) VALUES (?,?,?,?,?,?,?,?,?)`)
    .run(name, sub||'', emoji||'🌿', category_id, type||'Hybrid', thc||'', cbd||'', description||'', tag||'New');
...
app.post('/api/driver/orders/:id/deliver', requireAuth(['driver']), (req, res) => {
  const { id_verified } = req.body || {};
  if (!id_verified) return res.status(400).json({ error: 'must confirm ID verification at door' });
  db.prepare(`UPDATE orders SET status='delivered', delivered_at=CURRENT_TIMESTAMP WHERE id=? AND driver_id=?`).run(req.params.id, req.user.id);
  res.json({ ok: true });
});
